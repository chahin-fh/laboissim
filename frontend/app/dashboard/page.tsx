...
export default function DashboardPage() {
  ...
  const projectManagementRef = useRef<HTMLDivElement | null>(null)
  ...
  const handleEditProject = (project: any) => {
    ...
    // Scroll to the project management form for visibility
    setTimeout(() => {
      projectManagementRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }, 0)
  };
  ...
  const handleUpdateProject = async () => {
    ...
    if (projectForm.title && projectForm.description) {
      try {
        await updateProject({ id: editingProjectId, ...projectForm })
        // Upload any queued files to this project
        if (projectFilesQueue.length > 0) {
          try {
            await Promise.all(
              projectFilesQueue.map((file) => uploadProjectDocument(editingProjectId, file))
            )
          } catch (e) {
            console.error('Error uploading project documents during update:', e)
          }
        }
        const updatedProjects = await getProjects();
        setProjects(updatedProjects);
  ...
  const openProjectDetails = async (project: any) => {
    try {
      ...
      const docs = await getProjectDocuments(String(project.id))
      setSelectedProjectDocuments(docs)
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="border-b pb-4">
                          ...
                          <div className="space-y-3">
                              ...
                              >
                                ...
                                {projectFilesQueue.length > 0 && (
                                  <div className="text-sm text-gray-600 mt-2 text-left">
                                    {projectFilesQueue.map((file, index) => (
                            ...
                            >
                              ...
                              {editingProjectId ? 'Mettre à jour le projet' : 'Créer le projet'}
                            </Button>
                          </div>
                        </div>
            ...
            <Dialog open={!!selectedProjectForDetails} onOpenChange={(open) => {
              if (!open) {
              ...
              <DialogContent className="max-w-3xl">
                <DialogHeader>
                  <DialogTitle className="text-xl">
                    {selectedProjectForDetails?.title || 'Détails du projet'}
                  </DialogTitle>
                  <DialogDescription>
                    {selectedProjectForDetails?.created_by_name ? (
                      <span>Créé par {selectedProjectForDetails.created_by_name}</span>
                    ) : null}
                  </DialogDescription>
                </DialogHeader>
                {selectedProjectForDetails && (
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      <Badge
                        className={
                          selectedProjectForDetails.status === "planning"
                            ...
                            ? "bg-green-100 text-green-700"
                            : selectedProjectForDetails.status === "on_hold"
                            ? "bg-orange-100 text-orange-700"
                      ...
                      <Badge
                        className={
                          selectedProjectForDetails.priority === "low"
                            ? "bg-gray-100 text-gray-700"
                            : selectedProjectForDetails.priority === "medium"
                            ? "bg-blue-100 text-blue-700"
                            : selectedProjectForDetails.priority === "high"
                            ? "bg-orange-100 text-orange-700"
                            : "bg-red-100 text-red-700"
                        }
                      >
                    ...
                    <div>
                      ...
                      <p className="text-gray-700 text-sm">{selectedProjectForDetails.description}</p>
                    </div>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {selectedProjectForDetails.start_date
                            ? new Date(selectedProjectForDetails.start_date).toLocaleDateString('fr-FR')
                            : 'Non définie'}
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {selectedProjectForDetails.end_date
                            ? new Date(selectedProjectForDetails.end_date).toLocaleDateString('fr-FR')
                            : 'Non définie'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div>
                      <h4 className="font-medium text-gray-800 mb-1">Membres de l'équipe</h4>
                      <div className="flex flex-wrap gap-2">
                        {(selectedProjectForDetails.team_members_names || []).map((name: string, idx: number) => (
                      ...
                      {projectDetailsLoading ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {selectedProjectDocuments.map((doc) => (
                            <div key={doc.id} className="p-3 rounded-lg border bg-gray-50">
                              <div className="flex items-start justify-between">
                                ...
                                <div className="flex items-center gap-1 ml-2">
                                  <Button variant="ghost" size="sm" onClick={() => window.open(doc.file, '_blank')} className="text-blue-600 hover:text-blue-700">
                                    <Eye className="h-4 w-4" />
                                  </Button>
                                  <Button variant="ghost" size="sm" onClick={() => window.open(doc.file, '_blank')} className="text-green-600 hover:text-green-700">
