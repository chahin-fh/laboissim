const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://laboissim.onrender.com';
...
// Public version that doesn't require authentication
export const getPublicProjectDocuments = async (projectId: string): Promise<ProjectDocument[]> => {
  const response = await fetch(`${API_URL}/api/project-documents/?project_id=${projectId}`, {
    headers: {
      'Content-Type': 'application/json',
    },
